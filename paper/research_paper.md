...
To integrate the attestation pipeline with commercial games without modifying their source code, we use a dynamic interception hook. When the game client attempts to initialize the Easy Anti-Cheat or Epic Online Services (EOS) SDK, the dynamic linker loads our preloaded library (`eac_hook.so`). The hook intercepts the initialization call (`EOS_AntiCheatClient_AddNotifyMessageToServer`), opens a Unix-domain socket connection to a local daemon (`shim.py`), and blocks the game thread. The `shim.py` daemon collects the TPM report, posts it to the remote verification server, obtains the session token, and returns it to `eac_hook.so`. The hook then registers its own callbacks to verify subsequent SDK network messages, releasing the blocked thread only upon successful attestation.
...
While our proof-of-concept demonstrates the feasibility of TPM-rooted anti-cheat, several limitations must be addressed before production deployment.

### Latency and Kernel Requirements
The 15-second pipeline latency is a usability constraint. Spawning subprocesses and parsing large IMA logs is slow. Future work will rewrite the agent in Rust or C, utilizing direct kernel APIs (`/sys/kernel/security/ima/binary_runtime_measurements`) instead of parsing the ascii representations. Additionally, TPM-Attest requires a kernel compiled with `CONFIG_IMA=y` and boot parameters (`ima_policy=tcb`), which are not enabled by default on all consumer Linux distributions.

### AK Key Enrollment and PKI
Currently, the AK enrollment process is manual. In a production system, this requires a Public Key Infrastructure (PKI). The server must verify that the client's Attestation Key (AK) is bound to a genuine Endorsement Key (EK) certified by the TPM manufacturer's Certificate Authority (CA). This protects against software-emulated TPMs that generate fake signatures.

### IMA Log Compression
As system runtime increases, the IMA log grows, increasing Merkle tree calculation time. We plan to implement a streaming proof model where the client maintains the tree state incrementally, sending only the Merkle path proofs for new file executions, reducing network payload and computation overhead.

### Vendor Collaboration
Our ultimate goal is to approach major anti-cheat vendors (such as Epic Games and BattlEye) with our open-source prototype. By demonstrating that TPM-Attest provides equivalent or stronger security guarantees than kernel-level scanners without compromising Linux user privacy, we hope to encourage the official adoption of hardware-rooted remote attestation in commercial games.
